...
pub enum GovernanceInstruction {
    ...
    ///   2. `[signer]` Governance Authority (Token Owner or Governance Delegate)
    ///   3. `[writable]` Signatory Record Account
    ///   4. `[writable]` Beneficiary Account which would receive daltons from the disposed Signatory Record Account
    RemoveSignatory {
        #[allow(dead_code)]
    ...
    ///   4. `[writable]` Beneficiary Account which would receive daltons from the disposed ProposalTransaction account
    RemoveTransaction,
    ...
    ///   7. `[writable]` Optional Beneficiary account which would receive daltons when VoteRecord Account is disposed
    ///       It's required only when Proposal is still being voted on
    RelinquishVote,
