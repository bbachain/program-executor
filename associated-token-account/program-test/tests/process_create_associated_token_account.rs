...
async fn test_associated_token_address() {
    ...
    assert_eq!(associated_account.daltons, expected_token_account_balance);
}
...
async fn test_create_with_fewer_lamports() {
    ...
    // Transfer daltons into `associated_token_address` before creating it - enough to be
    ...
    // Check that the program adds the extra daltons
    let mut transaction = Transaction::new_with_payer(
        &[create_associated_token_account(
            &payer.pubkey(),
            &wallet_address,
...
async fn test_create_with_excess_lamports() {
    ...
    // Check that the program doesn't add any daltons
    let mut transaction = Transaction::new_with_payer(
        &[create_associated_token_account(
            &payer.pubkey(),
            &wallet_address,
