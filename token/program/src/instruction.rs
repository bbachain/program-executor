...
pub enum TokenInstruction<'a> {
    ...
    /// updates its amount field based on the account's underlying `daltons`.
    /// This is useful if a non-wrapped SOL account uses `system_instruction::transfer`
    /// to move daltons to a wrapped token account, and needs to have its token
    /// `amount` field updated.
    ///
    /// Accounts expected by this instruction:
    ///
    ///   0. `[writable]`  The native token account to sync with its underlying daltons.
    SyncNative,
